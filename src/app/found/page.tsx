  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-semibold">Found Items</h1>
          <p className="mt-1 text-sm text-slate-400">Items reported by people who found something.</p>
        </div>
        <Link href="/report/found" className="btn-primary">Report a found item</Link>
      </div>
      ...
      {error && (
        <p className="mt-8 text-sm text-red-400">
          We couldn&apos;t load found items right now. Please try again shortly.
        </p>
      )}

      {!error && items && items.length === 0 && (
        <div className="card mt-8 p-10 text-center">
          <p className="text-slate-300">No found items reported yet.</p>
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {items?.map((item) => (
          <ItemCard
            ...
            date={format(new Date(item.date_found), "MMM d, yyyy")}
