...
export default function ReportFoundPage() {
  ...
  // ── Product analytics: report funnel (mirrors the lost wizard) ────────────
  ...
  useEffect(() => {
    track("report_started", "reports", { kind: "found" });
  }, []);
  useEffect(() => {
    track("report_step", "reports", { kind: "found", step });
  }, [step]);
  useEffect(
    () => () => {
      if (!submittedRef.current) {
        track("report_abandoned", "reports", { kind: "found", last_step: stepRef.current });
        flushSync();
      }
    },
    []
  ...
  async function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      if (result?.error) {
        ...
        track("report_submit_error", "reports", { kind: "found", stage: "create" });
        return;
      }
      if (!result?.itemId) {
      ...
      if (uploadErr) {
        ...
        setError(uploadErr);
        track("report_submit_error", "reports", { kind: "found", stage: "upload" });
        return;
      }

      submittedRef.current = true;
      track("report_submitted", "reports", { kind: "found", photos: images.length });
          ...
          <div id="step-1" className={step === 1 ? "space-y-5" : "space-y-5 hidden"}>
              ...
              <div>
                <label htmlFor="distinguishingFeatures" className="label">
                  Private verification details <span className="font-normal text-slate-500">(optional)</span>
                </label>
                ...
                <p className="mt-1 flex items-start gap-1 text-xs text-slate-500">
                  ...
                  Hidden from the public listing — only someone who proves ownership can see these.
                </p>
              </div>
            </div>
