...
| ID | Item | Why | How |
|---|---|---|---|
| **V-1** | Confirm `consume_rate_limit` RPC exists in **production** Supabase | If missing, the limiter silently degrades to per-instance (weaker). Code logs it; verify proactively. | Supabase SQL editor: `select consume_rate_limit('test','127.0.0.1',1,1000);` → expect `true`, re-run → expect `false`. |
| **V-2** | Confirm Turnstile keys are set in **Vercel** env (not only `.env.local`) | Local keys don't deploy; the widget would render but verification is a no-op in prod. | Vercel → Settings → Environment Variables: `NEXT_PUBLIC_TURNSTILE_SITE_KEY`, `TURNSTILE_SECRET_KEY` → redeploy. |
